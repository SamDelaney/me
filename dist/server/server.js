...
async function loadVirtualModule(id) {
  switch (id) {
    case VIRTUAL_MODULES.startManifest:
      return await import("./assets/_tanstack-start-manifest_v-XZL5lNPJ.js");
...
const createServerRpc = (functionId, splitImportFn) => {
  const url = "/me/_serverFn/" + functionId;
...
function createStartHandler(cb) {
  const ROUTER_BASEPATH = "me";
  let startRoutesManifest = null;
  let startEntry = null;
  let routerEntry = null;
  const getEntries = async () => {
    if (routerEntry === null) {
      routerEntry = await import("./assets/router-B5mtVMlV.js").then((n) => n.r);
    }
    if (startEntry === null) {
    ...
    const requestHandlerMiddleware = handlerToMiddleware(
      async ({ context }) => {
        const response2 = await runWithStartContext(
          ...
          async () => {
            try {
              if (href.startsWith("/me/_serverFn/")) {
